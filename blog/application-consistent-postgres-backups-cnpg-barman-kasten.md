...
| Orchestrate it | **Kasten K10** | Trigger the DB backup *and* capture all the K8s resources |
...
```mermaid
flowchart LR
    P["Kasten K10 Policy run<br/>(schedule + retention)"]
    P --> BP
    P --> EX
    ...
    subgraph RES["RESOURCES — the rest of the app"]
      direction LR
      EX["K10 Snapshot + Export<br/>Secrets · Services ·<br/>Cluster spec · Backup objects"] --> KP[("Kasten profile<br/>kasten-backups")]
    end
    S3 -.-> R["Restore = Barman recovery cluster (data)<br/>+ K10 restore (resources) = whole app back"]
    KP -.-> R
```
...
![MinIO console showing the cnpg-barman bucket with base/ and wals/ prefixes](images/minio-cnpg-barman.png)
...
> "successful" backups were quietly un-restorable. Always verify:
> ```bash
...
## Part 3 — Orchestrate it all with Kasten K10
...
wires it up, so a plain namespace policy Just Works, and K10 runs the blueprint whenever it
...
`backup-pg-<timestamp>` CNPG Backup object appears, Barman completes it to S3, and K10
snapshots and exports the namespace's Kubernetes resources to the location profile.

![Kasten K10 dashboard showing the two policies and a successful run](images/k10-policies-run.png)
*The two policies in the K10 dashboard, and a green successful run of `postgres-backup`.*

![Kasten K10 restore point expanded to show the captured CNPG Backup object](images/k10-restorepoint.png)
